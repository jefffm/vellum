...
function buildWorkflow(): string {
  return [
    ...
    "6. Always `compile` with SVG output and verify before presenting the final result",
    "7. Only report success after `compile` returns an SVG or PDF artifact with no errors",
    "8. After a successful compile, update the tablature preview in the side panel via artifacts",
    "",
