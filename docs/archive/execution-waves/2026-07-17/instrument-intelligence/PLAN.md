...
```mermaid
flowchart TD
  T63["T63 pre-HITL readiness"] --> T64["T64 curator decision"]
  T63 --> T102["T102 maintainer decision"]
  T102 --> T105["T105 verify decision and activate"]
  T64 --> T106["T106 precommit adjudication"]
  T105 --> T106
  T106 -- "nonpassing" --> RP["AFK repair or successor decision package"]
  RP --> T64
  RP --> T102
  T106 -- "precommit ready" --> T82["T82 human truth commitments"]
  T82 --> T103["T103 verify truth and freeze"]
  T103 -- "nonpassing" --> RT["AFK repair or successor truth package"]
  RT --> T82
  T103 -- "freeze complete" --> T83["T83 automatic sealed run"]
  T83 --> T84["T84 adjudication and repair dispatch"]
  T84 -- "passing current round" --> T85["T85 Machine closure audit"]
  T84 -- "repair-dispatch failure" --> RQ["append AFK repair tracer"]
  T84 -- "blocked, incomplete, or fixture replacement" --> GQ
  RQ --> GQ["new T83/T84 execution generations"]
  GQ --> T83
  T85 -- "machine complete" --> T81["T81 exact review packages"]
  T85 -- "repairable closure failure" --> RM["append AFK repair tracer"]
  T85 -- "blocked or incomplete" --> AM["policy-bound T85 retry generation"]
  AM --> T85
  RM --> GM["fresh affected-slice generations"]
  GM --> T85
  T81 --> T107["T107 lyric applicability"]
  T81 --> REV["unconditional independent reviews"]
  T107 -- "lyrics applicable" --> T94["T94 lyric review"]
  T107 -- "not applicable" --> T69
  T107 -- "blocked or incomplete" --> T69
  T94 --> T69
  REV --> T69["T69 review aggregation"]
  T69 -- "all required reviews pass" --> T87["T87 Release closure audit"]
  T69 -- "repair-dispatch finding" --> RR["append AFK repair tracer"]
  T69 -- "successor review or retry" --> SR["fresh authorized review/retry generation"]
  SR --> T69
  RR --> GR["fresh prescribed rejoin and affected descendants"]
  GR --> T87
  T87 -- "release complete" --> DONE["Release Complete"]
  T87 -- "repairable closure failure" --> RL["append AFK repair tracer"]
  T87 -- "blocked or incomplete" --> AR["policy-bound T87 retry generation"]
  AR --> T87
  RL --> GL["fresh affected-slice generations"]
  GL --> T87
  T81 -- "Owner defers reviews" --> T86["T86 provisional stop"]
  T69 -- "Owner pauses after nonpass" --> T86
```
