...
Requirement IDs: II-MUS-009, II-SEED-004, II-EXEC-006A, II-MC-014
...
- [ ] Each sign/group/span has exact source segment, bass/time/voice anchor, uncertainty, correction lineage, and generated-realization mapping slots.
...
- Conditional: `npm run test:browser -- test/browser/instrument-intelligence/T36-optical-figured-bass-truth.spec.ts`; `npm run eval:fast`; `nix develop --command bash -c 'npm run sandbox:lilypond:verify && npm run eval:render && npm run eval:playback'`.
